...
def searchNNClusts(sel_samples):
    ...
    Returns:
    --------
    Numpy array of shape (6,2). Each row corresponds to a spatial window (0-10km, 10-110km, ..., 410-510km). For each row, the first element corresponds to the number
...
base_seed = int(time.time()) + 1000*task_id
filename = "results_" + str(task_id) + ".csv"

print("The seed number is: " + str(base_seed))

start = time.time()
tau = simClustSample(base_seed, 10, 1e3, 10, 500, 500, 20)
for i in range(1,200):
    tau = np.vstack([tau, simClustSample(base_seed + i, 10, 1e3, 10, 500, 500, 20)])
    if i % 25 == 0:
        np.savetxt(filename, tau, delimiter=",")

np.savetxt(filename, tau, delimiter=",")

print("Job done in " + str(int(time.time() - start)) + "s.")